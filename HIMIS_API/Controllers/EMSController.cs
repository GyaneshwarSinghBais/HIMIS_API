...
) as uploadStatus,A.CreatedOn,SUM(B.FINAL_QTY) * isnull(m.estimated_cost,0) IndentValue
 FROM INDENT_CONSOLIDATION A 
inner join  INDENT_CONS_ITEMS B ON (B.INDENT_CONSOLIDATED_ID = A.INDENT_CONSOLIDATION_ID)
inner join masitems	m on m.item_id=B.item_id
...
GROUP BY  b.indent_cons_items_id,b.item_id,A.INDENT_CONSOLIDATION_ID,A.USER_ID,A.DIRECTORATE_ID,A.FINANCIAL_YEAR_ID,A.STATUS,A.CONSOLIDATED_DATE,A.indent_con_no,A.CreatedOn,A.description,m.estimated_cost
)x  ";
        ...
        //https://localhost:7247/api/EMS/EqToBeTenderDetail
        [HttpGet("EqToBeTenderDetail")]
        public async Task<ActionResult<IEnumerable<EqToBeTenderDetailDTO>>> EqToBeTenderDetail()
        {


            string query = $@"   SELECT A.INDENT_CONSOLIDATION_ID,A.indent_con_no,CONVERT(VARCHAR(10),A.CONSOLIDATED_DATE,103) AS CONSOLIDATED_DATE,A.description
,b.item_id,b.indent_cons_items_id
,item_code,item_code_as_per_tender,item_name,item_desc
,SUM(B.PROPOSED_QTY) AS PROPOSED_QTY
,SUM(B.FINAL_QTY) AS FINAL_QTY,
isnull(m.estimated_cost,0) *SUM(B.FINAL_QTY) IndentValue
,fy.YEAR
,A.USER_ID,A.DIRECTORATE_ID,A.FINANCIAL_YEAR_ID,facility_aut_name,facility_aut_code

,(case when A.STATUS='I' then 'Incomplete' when A.STATUS='C' then 'Completed' else '' end ) EStatus,
(
select (case when FileName is null then 'Not Uploaded' else 'Uploaded' end) UStatus from INDENT_CONSOLIDATION where indent_consolidation_id = A.INDENT_CONSOLIDATION_ID
) as uploadStatus,A.CreatedOn
 FROM INDENT_CONSOLIDATION A 
inner join  INDENT_CONS_ITEMS B ON (B.INDENT_CONSOLIDATED_ID = A.INDENT_CONSOLIDATION_ID)	
inner join masitems	m on m.item_id=B.item_id
inner join mas_financial_year  fy on fy.financial_year_id	=A.financial_year_id
inner join  facility_aut fa on fa.facility_aut_id=A.directorate_id
WHERE  a.financial_year_id >=18  
...
GROUP BY  b.indent_cons_items_id,b.item_id,A.INDENT_CONSOLIDATION_ID,A.USER_ID
,A.DIRECTORATE_ID,A.FINANCIAL_YEAR_ID,A.STATUS,A.CONSOLIDATED_DATE,A.indent_con_no
,A.CreatedOn,A.description,m.estimated_cost,item_code,item_code_as_per_tender,item_name,item_desc,fy.YEAR,facility_aut_name,facility_aut_code  ";

            var result = await _context.EqToBeTenderDetailDbSet
                .FromSqlRaw(query)
                .ToListAsync();
