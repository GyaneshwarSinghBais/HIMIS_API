...
namespace HIMIS_API.Controllers
{
    ...
    public class FinancialController : Controller
    {
        ...
        public FinancialController(DbContextData context)
        {
            _context = context;
        }
        [HttpGet("HandoverAbstract")]
        public async Task<ActionResult<IEnumerable<HandOverAbstractDTO>>> HandoverAbstract(string RPType,string dashid,string divisionid,string districtid,string SWId, string fromdt,string todt)
        {
            string? whereClause = "";
            if (divisionid != "0")
            {
...
order by  round(sum(TVC)/100,2) desc ";
        ...
        public async Task<ActionResult<IEnumerable<WorkorderpendingdetailsDTO>>> getHandoverDetails(string dashid,string divisionId, string mainSchemeId, string distid,string SWId)
        {
            string? whereClause = "";
            if (divisionId != "0")
            {
...
inner join AgreementDetails  n on n.work_id= w.work_id
 inner join MasTender t on t.tenderid= n.tenderid
